...
public class JoinCommandHandler implements CommandHandler {
    ...
    public Mono<Void> handle(ChatInputInteractionEvent event) {
        return event.deferReply().flatMap(unused -> internalHandle(event));
    }

    public Mono<Void> internalHandle(ChatInputInteractionEvent event) {
        ...
        return Mono.justOrEmpty(event.getInteraction().getGuildId())
                ...
                .flatMap(guildData -> Mono.justOrEmpty(event.getInteraction().getMember())
                        ...
                        .map(guildData::setVoiceConnection)
                        ...
                        .doOnSuccess(channel -> {
                            ...
                            InteractionFollowupCreateSpec followupCreateSpec = InteractionFollowupCreateSpec.builder()
                                    .addEmbed(Menmu.createSuccessEmbedSpec(message)).build();
                            event.createFollowup(followupCreateSpec);
                        }).onErrorMap(error -> new CommandExecutionException("join", ErrorType.VOICE_CONNECTION_ERROR, error)))).then();
    }
